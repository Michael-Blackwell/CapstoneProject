"""
Author: mike
Created: 3/27/22

"""

import tensorflow_addons as tfa
import tensorboard
import pandas as pd
from ModelFunctions import *
from tensorflow.keras.layers import Conv2D, Dropout, BatchNormalization, MultiHeadAttention, ReLU
from tensorflow import keras
from tensorflow.keras.layers import Conv2D, Softmax, Permute, BatchNormalization, Activation, Dropout, MaxPooling2D, Conv2DTranspose




def INF(B, H, W):
    return -torch.diag(torch.tensor(float("inf")).cuda().repeat(H), 0).unsqueeze(0).repeat(B * W, 1, 1)


class CrissCrossAttention(tf.keras.Model):
    """ Criss-Cross Attention Module"""

    def __init__(self, in_dim):
        super(CrissCrossAttention, self).__init__()
        # ToDo need to revisit the filters to ensure the correct element of in_dim is being used.
        self.query_conv = Conv2D(filters=in_dim // 8, kernel_size=1)
        self.key_conv = Conv2D(filters=in_dim // 8, kernel_size=1)
        self.value_conv = Conv2D(filters=in_dim, kernel_size=1)
        self.softmax = Softmax(dim=3)
        self.INF = INF
        self.gamma = tf.constant(0)

    def forward(self, x):
        m_batchsize, _, height, width = x.size()
        proj_query = self.query_conv(x)

        # Prepare Query
        # H
        proj_query_H = Permute((0, 3, 1, 2))(proj_query)
        proj_query_H = tf.reshape(proj_query_H, (m_batchsize * width, -1, height), name='Query_W_Reshape')
        proj_query_H = Permute((0, 2, 1))(proj_query_H)
        # W
        proj_query_W = Permute((0, 2, 1, 3))(proj_query)
        proj_query_W = tf.reshape(proj_query_W, (m_batchsize * height, -1, width), name='Query_W_reshape')
        proj_query_W = Permute((0, 2, 1))(proj_query_W)

        # proj_query_H = proj_query.permute(0, 3, 1, 2).contiguous().view(m_batchsize * width, -1, height).permute(0, 2, 1)
        # proj_query_W = proj_query.permute(0, 2, 1, 3).contiguous().view(m_batchsize * height, -1, width).permute(0, 2, 1)

        # Prepare Key
        # proj_key_H = proj_key.permute(0, 3, 1, 2).contiguous().view(m_batchsize * width, -1, height)
        # proj_key_W = proj_key.permute(0, 2, 1, 3).contiguous().view(m_batchsize * height, -1, width)
        proj_key = self.key_conv(x)
        # H
        proj_key_H = Permute((0, 3, 1, 2))(proj_key)
        proj_key_H = tf.reshape(proj_key_H, (m_batchsize * width, -1, height), name='key_H_reshape')
        # W
        proj_key_W = Permute((0, 2, 1, 3))(proj_key)
        proj_key_W = tf.reshape(proj_key_W, (m_batchsize * height, -1, width), name='key_W_reshape')

        # Prepare Value
        # proj_value_H = proj_value.permute(0, 3, 1, 2).contiguous().view(m_batchsize * width, -1, height)
        # proj_value_W = proj_value.permute(0, 2, 1, 3).contiguous().view(m_batchsize * height, -1, width)
        proj_value = self.value_conv(x)
        # H
        proj_value_H = Permute((0, 3, 1, 2))(proj_value)
        proj_value_H = tf.reshape(proj_value_H, (m_batchsize * width, -1, height))
        # W
        proj_value_W = Permute((0, 2, 1, 3))(proj_value)
        proj_value_W = tf.reshape(proj_value_W, (m_batchsize * height, -1, width))

        # Energy
        temp = self.INF(m_batchsize, height, width)
        # torch.bmm(proj_query_H, proj_key_H) + temp
        # H
        energy_H = tf.keras.layers.Dot(axes=(1, 2))([proj_query_H, proj_key_H]) + temp
        energy_H = tf.reshape(energy_H, (m_batchsize, width, height, height))
        energy_H = Permute((0, 2, 1, 3))(energy_H)
        # W
        energy_W = tf.keras.layers.Dot(axes=(1, 2))(proj_query_W, proj_key_W)
        energy_W = tf.reshape(energy_W, (m_batchsize, height, width, width))

        concate = self.softmax(tf.concat([energy_H, energy_W], 3))

        # Attention
        # H
        att_H = Permute((0, 2, 1, 3))(concate[:, :, :, 0:height])  .view(m_batchsize * width, height, height)
        att_H = tf.reshape(att_H, (m_batchsize * width, height, height))
        # W
        att_W = tf.reshape(concate[:, :, :, height:height + width], (m_batchsize * height, width, width))

        # Out
        # H
        out_H = torch.bmm(proj_value_H, att_H.permute(0, 2, 1)).view(m_batchsize, width, -1, height).permute(0, 2, 3, 1)
        out_W = torch.bmm(proj_value_W, att_W.permute(0, 2, 1)).view(m_batchsize, height, -1, width).permute(0, 2, 1, 3)
        # print(out_H.size(),out_W.size())
        return self.gamma * (out_H + out_W) + x
